# This file should contain all the record creation needed to seed the database with its default values.
# The data can then be loaded with the rails db:seed command (or created alongside the database with db:setup).
#
# Examples:
#
#   movies = Movie.create([{ name: 'Star Wars' }, { name: 'Lord of the Rings' }])
#   Character.create(name: 'Luke', movie: movies.first)

listings = [
  {
    "id": 1,
    "imageUrl": "https://i.kinja-img.com/gawker-media/image/upload/s--vYNPTkDt--/c_scale,fl_progressive,q_80,w_800/198qdy1m0jf7mjpg.jpg",
    "locationName": "Middle Earth",
    "price": 400,
    "title": "Upscale Lair in Quiet Neighborhood",
    "likes": 18
  },
  {
    "id": 2,
    "imageUrl": "https://66.media.tumblr.com/184f1c4149e8618763346cc0fe6acc37/tumblr_mvx9ut2WEe1sh566ao1_1280.jpg",
    "locationName": "Winterfell",
    "price": 200,
    "title": "Historic Castle in 'Cool' Uptown District",
    "likes": 10
  },
  {
    "id": 3,
    "imageUrl": "https://thecollegepost.com/wp-content/uploads/2019/12/main-building-750x430.jpg",
    "locationName": "Philadelphia",
    "price": 123,
    "title": "Student-Friendly Downtown Abode",
    "likes": 1
  },
  {
    "id": 4,
    "imageUrl": "https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTtvRFkcJwMhMhX3MpRSh9rE2H13ckTtF1RSNp2VwU1EeESpWa_&usqp=CAU.jpg",
    "locationName": "The void beyond time",
    "price": 0,
    "title": "Switch places with me",
    "likes": 5
  },
  {
    "id": 5,
    "imageUrl": "https://vignette.wikia.nocookie.net/lotr/images/e/e4/Vlcsnap-2013-05-19-19h49m07s0.png/revision/latest?cb=20130519155935",
    "locationName": "Middle Earth",
    "price": 10,
    "title": "Humble Home in Hobbiton",
    "likes": 2
  }
]

listings.each do |listing_hash|
  location = Location.find_or_create_by(name: listing_hash[:locationName])

  Listing.create(
    title: listing_hash[:title],
    image_url: listing_hash[:imageUrl],
    price: listing_hash[:price],
    likes: listing_hash[:likes],
    location: location
  )
end

puts "Created #{Listing.count} listings"